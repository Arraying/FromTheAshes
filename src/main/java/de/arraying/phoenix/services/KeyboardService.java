...
public class KeyboardService extends Service {
    ...
    public void keypress(int key, int action) {
        ...
        // Only change gamma if they have no screen (i.e. they are in game, no furnace, inventory, etc.).
        if (action == GLFW.GLFW_RELEASE && key == GLFW.GLFW_KEY_G && client.currentScreen == null) {
            phoenix.getHudService().toggleGamma();
        }
        // Only zoom if they have no screen.
